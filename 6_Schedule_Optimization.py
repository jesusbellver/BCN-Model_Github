...
Optimizes intervention schedules to minimize mosquito prevalence while 
respecting operational constraints (max zones per day, max days per week).

Uses numba-accelerated simulation core from 4_Validation_plots.py.
Parallelizes neighbor evaluation for faster convergence.

Usage:
    python 6_Schedule_Optimization.py <year>
    
Example:
    python 6_Schedule_Optimization.py 2020
"""
...
# Common to all betas and years:
#   - ITERS_ANNEALING=5000: never reached — early stopping always triggers first.
#   - C_T=0.99940: geometric cooling rate, same for all betas.
#   - EPS stepped reduction based on consecutive unproductive iterations:
#     Phase 0: eps_t=4, eps_p=2  (initial exploration)
#     Phase 1: eps_t=2, eps_p=1  (after 50 consecutive unproductive)
#     Phase 2: eps_t=1, eps_p=1  (after 50 more consecutive unproductive)
#     Stop:    after 50 more consecutive unproductive in phase 2.
#
# INIT_T is beta-dependent: calibrated to ~2× empirical median|dJ|,
# giving ~60% P(accept uphill) at start. Year does not affect INIT_T.
...
if BETA_SIM == 0:
    # Safety fallback; use 6_Schedule_Optimization_B0.py for beta=0.
    INIT_T = 0.072
elif BETA_SIM < 0.3:
    # PER-dominated. Empirical med|dJ| ~ 0.003
    INIT_T = 0.0063
elif 0.3 <= BETA_SIM < 0.5:
    # Near-balanced. Empirical med|dJ| ~ 0.005
    INIT_T = 0.0112
elif 0.5 <= BETA_SIM < 0.7:
    # INT starts dominating. Empirical med|dJ| ~ 0.009
    INIT_T = 0.0184
elif 0.7 <= BETA_SIM < 1.0:
    # INT dominant. Empirical med|dJ| ~ 0.011
    INIT_T = 0.0230
else:  # beta = 1.0
    # Pure INT. Empirical med|dJ| ~ 0.016
    INIT_T = 0.0363
...
def simulate_with_schedule(u0, n_drains, n_days, M_values, M_col_indices, M_row_starts,
    ...
    """
    Full simulation with scheduled interventions (CSR M format).
...
# SCHEDULE GENERATION
# ============================================================================
# Each zone has init_time (when to start) and periodicity (how often to repeat)

def generate_schedule(init_times, periodicity, n_zones, end_season):
    """
    Generate intervention days for each zone.
    Like Julia: Schedule[zone] = [init_times[z] + k * periodicity[z] for k in 0:n_interventions]
    """
    schedule = []
    counts = np.zeros(n_zones, dtype=np.int32)
    
    # First pass: calculate how many interventions each zone needs
    max_interventions = 0
    for z in range(n_zones):
        t0 = int(init_times[z])
...
def check_schedule(schedule, counts, n_zones, init_season, end_season, max_zones_day, max_days_week):
    ...
    # Check max days per week (Julia-style sliding window)
...
def random_schedule(n_zones, init_season, end_season, max_zones_day, max_days_week):
    ...
    for _ in range(500):
        ...
        # Julia: periodicity = 35 + (season_length - 35)*rand()
        periodicity = 20 + 30 * np.random.rand(n_zones)
        periodicity = np.round(periodicity)
    ...
    # Fallback
    init_times = np.array([init_season + 10 + z * 5 for z in range(n_zones)], dtype=np.float64) + 0.5
    periodicity = np.full(n_zones, 42.0)
    schedule, counts = generate_schedule(init_times, periodicity, n_zones, end_season)
    return init_times, periodicity, schedule, counts
...
def update_schedule(init_times_0, periodicity_0, eps_t, eps_p, n_zones, init_season, end_season,
    ...
    """
    Generate a neighbor by perturbing parameters (Julia-style).
    Perturbs ALL zones by ±eps_t or ±eps_p.
    """
    for _ in range(100):
        # +eps or -eps for each zone
        signs_t = 2 * np.round(np.random.rand(n_zones)) - 1
        signs_p = 2 * np.round(np.random.rand(n_zones)) - 1
        
        new_init = init_times_0 + eps_t * signs_t
        new_period = periodicity_0 + eps_p * signs_p
        
        # Clamp (Julia-style)
        new_init = np.clip(new_init, init_season + 0.5, init_season + 61 + 0.5)
        new_period = np.clip(new_period, 1, end_season - init_season + 1)
        
        schedule, counts = generate_schedule(new_init, new_period, n_zones, end_season)
        
        if check_schedule(schedule, counts, n_zones, init_season, end_season, max_zones_day, max_days_week):
            return new_init, new_period, schedule, counts
...
# INT: mosquito integral (normalized)
# PER: number of treatments (normalized)
...
def cost_function(integral, schedule, counts, n_zones, beta, norm_dict, init_season, end_season):
    ...
    INT: normalized integral (fewer mosquitoes = lower)
    PER: normalized treatment count (fewer treatments = lower)
    """
    ...
    PER = n_treatments / norm_dict["per_ASPB"]
...
# Spatial coupling matrix M in CSR format (zone-restricted, cache-friendly)
print("Building spatial coupling matrix (CSR)...")

@jit(nopython=True, cache=True)
def generate_M_csr(D, alphas, drain_to_zone, zone_members, zone_starts, n):
    """Generate zone-restricted M in CSR format. Same values as dense, sequential access."""
    # Count nnz per row
    nnz_per_row = np.zeros(n, dtype=np.int64)
    for i in range(n):
        z = drain_to_zone[i]
        nnz_per_row[i] = zone_starts[z + 1] - zone_starts[z]
    
    total_nnz = np.sum(nnz_per_row)
    row_starts = np.zeros(n + 1, dtype=np.int64)
    for i in range(n):
        row_starts[i + 1] = row_starts[i] + nnz_per_row[i]
    
    values = np.zeros(total_nnz, dtype=np.float64)
    col_indices = np.zeros(total_nnz, dtype=np.int64)
    ...
    for i in range(n):
        ...
        for k in range(zone_starts[z], zone_starts[z + 1]):
            j = zone_members[k]
            values[idx] = np.exp(-alpha_i * D[i, j])
            col_indices[idx] = j
            idx += 1
    
    return values, col_indices, row_starts

M_values, M_col_indices, M_row_starts = generate_M_csr(D_global, ALPHAS, drain_to_zone, zone_members, zone_starts_arr, n_drains)
print(f"  M CSR: {len(M_values)} nonzeros (from {n_drains}x{n_drains} = {n_drains*n_drains} entries)")
...
def _init_worker(M_values_arr, M_col_indices_arr, M_row_starts_arr,
    ...
    """Initialize worker process with shared data (CSR format for M)."""
    global _worker_data
    _worker_data = {
        'M_values': M_values_arr, 'M_col_indices': M_col_indices_arr,
        'M_row_starts': M_row_starts_arr,
...
def run_simulated_annealing(beta, executor, run_id=1):
    ...
    print(f"Finding initial schedule with J < {THRESHOLD_J}...")
    ...
    if len(candidates) > 0:
        ...
        print(f"Using best from first 50: J={J0[0]:.4f} (from {len(candidates)} below threshold)")
    else:
        ...
        for attempt in range(51, MAX_INIT_ATTEMPTS + 1):
            ...
            if J_temp[0] < THRESHOLD_J:
                ...
                print(f"Found J={J0[0]:.4f} < {THRESHOLD_J} at attempt {attempt}")
                found = True
                break
        ...
        if not found:
            ...
            print(f"No solution < {THRESHOLD_J} in {MAX_INIT_ATTEMPTS} attempts, using best: J={J0[0]:.4f}")
    ...
    Temp = INIT_T
    unproductive_iters = 0  # Count consecutive batches with no acceptance OR no J change
    last_J = J0[0]  # Track last accepted J
    
    # Stepped eps reduction
    eps_phase = 0
    eps_t = EPS_SCHEDULE[0][0]
    eps_p = EPS_SCHEDULE[0][1]
    ...
    while valid_iters < ITERS_ANNEALING:
        ...
        # Metropolis: find best accepted neighbor (only among valid ones)
        accepted = None
        for result, args in valid_neighbors:
            new_J = result[0]
            dE = new_J - J0[0]
            if dE < 0 or np.random.rand() < np.exp(-dE / Temp):
                if accepted is None or new_J < accepted[0][0]:
        ...
        if accepted is not None:
            ...
            # Check if this acceptance was productive (J changed)
            if J0[0] != last_J:
                last_J = J0[0]
                unproductive_iters = 0  # Reset: we made progress
            else:
                unproductive_iters += 1  # Accepted but J didn't change
        else:
    ...
    # Best result
    best_step = min(Results.keys(), key=lambda k: Results[k][0][0])
...
for i, results in enumerate(all_results, 1):
    ...
    print(f"Run {i}: Best J={best[0][0]:.4f} at step {best_step}, total steps: {total_steps}")
...
for i, results in enumerate(all_results):
    best_step = min(results.keys(), key=lambda k: results[k][0][0])
    cost = results[best_step][0][0]
    if cost < best_cost:
        best_cost = cost
        best_run = i + 1

print(f"\n*** Overall best: Run {best_run} with J={best_cost:.4f} ***")
